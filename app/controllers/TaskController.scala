...
import models.{BraveNews, Task}
import play.api.libs.json.Json
import play.api.libs.streams.ActorFlow
import play.api.mvc._
import play.api.mvc.WebSocket.MessageFlowTransformer
import play.api.libs.concurrent.Execution.Implicits.defaultContext
import service.{BraveNewsService, TaskService}
import akka.actor.ActorSystem
import forms.TaskForm

@Singleton
class TaskController @Inject()(implicit actorSystem: ActorSystem, materializer: Materializer, taskService: TaskService, braveNewsService: BraveNewsService) extends Controller {
  ...
  def taskJson = Action.async { request =>
    braveNewsService.addBraveNews(new BraveNews(id = 0, title = "", detail = "test", period = "asota", url = "https://google.com"))
    braveNewsService.getAllBraveNews().map { case tasks => Ok(Json.toJson(tasks))
    }
  }
}
